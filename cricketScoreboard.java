/***
 * The code represents a simple cricket scoreboard system implemented in Java. It allows users to manage cricket teams, start matches, and track scores. Here's a breakdown of the key components and their functionalities:

1.Classes and Their Responsibilities:

Team: Represents a cricket team, containing its name, a list of players, and the number of players.
Player: Represents an individual player with attributes for name, role (e.g., bowler, batsman), and individual score.
Match: Manages the state of a cricket match, including teams, overs, current over, scores, wickets, batsmen, bowlers, and tracking the number of overs bowled by each player.
cricketScoreboard Class:

2.Attributes:
teams: Array to store up to 10 teams.
numTeams: Number of teams currently in the system.
scanner: Scanner object for user input.
3.Methods:
displayMenu(): Displays the main menu options.
viewTeams(): Shows the list of added teams.
addTeam(): Allows adding a new team, including input for player details.
startMatch(): Starts a new match between two selected teams and sets up the initial conditions.
playMatch(): Manages the flow of the match, including handling deliveries, updating scores, and managing wickets.
rotateStrike(): Handles the rotation of strike based on runs scored.
changeBowler(): Allows changing the bowler after an over.
getBowlerIndex(): Retrieves the index of a bowler in a team.
displayResults(): Displays the results of the match, including scores and winner.
displayActivePlayers(): Shows the list of active players in a team (ensuring there are at least 11 players).

4.Methodology:
Interactive Command-Line Interface: The code uses a console-based menu system to interact with the user. Users can choose to view teams, add new teams, start matches, or exit the program.
Error Handling: Basic error handling is provided for user input, such as catching InputMismatchException when entering the number of players.
Match Simulation: The playMatch() method simulates a cricket match, updating scores, handling different types of deliveries, managing wickets, and switching bowlers.
Recursion: The addTeam() method uses recursion to allow adding multiple teams in sequence based on user input.
